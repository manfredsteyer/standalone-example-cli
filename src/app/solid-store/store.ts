...
import { $PROXY, $TRACK } from "./symbols";
...
export function getDataNode(nodes: DataNodes, property: PropertyKey, value: any) {
  const n = nodes[property] || (nodes[property] = createDataNode(value));
  return n;
}
...
export function trackSelf(target: StoreNode) {
  if (/*getListener()*/ true) {
    ...
    ((nodes as any)._ || ( (nodes as any)._ = createDataNode()))();
  }
}
